...
%imglist2 = dir([filedir2,'*.tif']);
N = natsortfiles({imglist1.name});
Nimg = length(imglist1);
%img_ncent = nstart-ncent+m1/2;
img_center = (nstart-ncent+m1/2)*dpix_m;
xlo = struct2array(load('xloylo.mat', 'xlo'));%293����
ylo = struct2array(load('xloylo.mat', 'ylo'));%293�
load('./expt_lit-8.mat');

%{
N2 = natsortfiles({imglist2.name});
Nimg2 = length(imglist2);
Iall10 = zeros(2160,2560,Nimg2,'uint16');
for mm1 = 1:Nimg2
    fn2 = [filedir2,N2{mm1}];  %added for sorting purpose
    disp(fn2);
    I10 = double(imread(fn2));
     bk1 = mean2(double(I10(1:100,1:100)));
    bk2= mean2(double(I10(492:600,2380:2520)));
    Ibk10(mm1) = mean([bk1,bk2]);
    if Ibk10(mm1)>24000 && mm1 >2
             Ibk10(mm1) = Ibk10(mm1-1);
      end
...
%clear I Iall
%I9 = double(Iall10(nstart(1):nstart(1)+m1-1,nstart(2):nstart(2)+m1-1,:));

newimSeqLowRes = double(Iallend);


% 
objectlow_first = sqrt(abs(newimSeqLowRes(:,:,146)));
figure(85);
...
dd =sqrt(((LEDx-13).*LEDgap-img_center(2)).^2+((LEDy-14).*LEDgap-img_center(1)).^2+LEDheight^2);
kxx=((LEDy-14)*LEDgap-img_center(1))./dd/waveLength;
kyy=((LEDx-13)*LEDgap-img_center(2))./dd/waveLength;
kall=kxx.^2+kyy.^2;
kall_sort_1=sort(kall,1);
[LED_list1,LED_list2]=sort(kall_sort_1(1,:),2);
%[LED_list1,LED_list2]=sort(sum(kall_sort_1,1));

%objectlow = sqrt(abs(newimSeqLowRes2(:,:,mm(1)));
%objectRecoverFT = F(sqrt(newimSeqLowRes(:,:,LED_list2(1))));
%objectRecoverFT = F(sqrt(newimSeqLowRes(:,:,mm(1))));
%objectRecoverFT = upsamp(objectRecoverFT);
load('./img_re.mat');

%{
I_RecoverFT2 = Ft(upsamp(F(sqrt((abs(newimSeqLowRes(:,:,33)))))));
...
fprintf('| %2d   | %.2e |\n',iter,err2);
    ...
    for tt=1:50
        ...
        gamma3=.1;%pupil
        for i4=1:Nimg2
            dd=sqrt(((xlit(mm(i4))-13)*LEDgap-img_center(1))^2+((ylit(mm(i4))-14)*LEDgap-img_center(2))^2+LEDheight^2);
            kxx2=((ylit(mm(i4))-14)*LEDgap-img_center(2))./dd/waveLength;
            kyy2=((xlit(mm(i4))-13)*LEDgap-img_center(1))./dd/waveLength;
    
   
            kxc2=round((n+1)/2+kxx2./dkx);%
            kyc2=round((m+1)/2+kyy2./dky);
            kyl2=round(kyc2-(m1-1)/2);kyh2=round(kyc2+(m1-1)/2);
            kxl2=round(kxc2-(n1-1)/2);kxh2=round(kxc2+(n1-1)/2);
       
            lowResFT2 = zeros(m1,m1);
            im_lowRes2 = zeros(m1,m1);
            im_lowRes_oI=0;
        
            lowResFT2(:,:)=objectRecoverFT(kyl2:kyh2,kxl2:kxh2).*pupil;
            im_lowRes2(:,:) = (ifft2(ifftshift(lowResFT2(:,:))));
            im_lowRes_oI=im_lowRes_oI+(abs(im_lowRes2(:,:))).^2;
            
      
            im_lowResk = zeros(m1,m1,LED_num);

            im_lowResk = sqrt(abs((I9(:,:,i4))))./sqrt(abs(im_lowRes_oI)).*im_lowRes2;

            lowResFT=fftshift(fft2(im_lowResk));%
            OP_diff=lowResFT-lowResFT2;
            ...
            sumpupil(kyl2:kyh2,kxl2:kxh2) = sumpupil(kyl2:kyh2,kxl2:kxh2)+abs(pupil).^2;
            
            
            %dobject=dobject+abs(pupil2).*conj(pupil2).*(OP_diff2(:,:,kk));
            dobject(kyl2:kyh2,kxl2:kxh2)=dobject(kyl2:kyh2,kxl2:kxh2)+abs(pupil).*conj(pupil).*(OP_diff);
     
            dpupil =  dpupil+(abs(OFTdown).*conj(OFTdown)).*(OP_diff);
...
for tt=1:loop
    ...
    for i4 = 10:Nimg
        ...
        %i3=mm(i4);
        %for ijk=1:8
        [i2,i3]=find(LED8293==mm(i4));
        kxc2=round((n+1)/2+kxx(:,i3(ijk))./dkx);%
        kyc2=round((m+1)/2+kyy(:,i3(ijk))./dky);
        kyl2=round(kyc2-(m1-1)/2);kyh2=round(kyc2+(m1-1)/2);
        kxl2=round(kxc2-(n1-1)/2);kxh2=round(kxc2+(n1-1)/2);
       
        lowResFT2 = zeros(m1,m1,LED_num);
        im_lowRes2 = zeros(m1,m1,LED_num);
        im_lowRes_oI=0;
        
        for r=1:LED_num
            
            lowResFT2(:,:,r)=objectRecoverFT(kyl2(r,:):kyh2(r,:),kxl2(r,:):kxh2(r,:)).*pupil;
            im_lowRes2(:,:,r) = (ifft2(ifftshift(lowResFT2(:,:,r))));
            im_lowRes_oI=im_lowRes_oI+(abs(im_lowRes2(:,:,r))).^2;
            
        end
        im_lowResk = zeros(m1,m1,LED_num);
        
        im_lowResk = sqrt(abs((newimSeqLowRes(:,:,i3))))./sqrt(abs(im_lowRes_oI)).*im_lowRes2;
        ...
        for kk=1:LED_num
            ...
            OFTdown = objectRecoverFT(kyl2(kk,:):kyh2(kk,:),kxl2(kk,:):kxh2(kk,:));
            sumobject = sumobject+abs(OFTdown).^2;
            
            
            sumpupil(kyl2(kk,:):kyh2(kk,:),kxl2(kk,:):kxh2(kk,:)) = sumpupil(kyl2(kk,:):kyh2(kk,:),kxl2(kk,:):kxh2(kk,:))+abs(pupil).^2;
            
            
            %dobject=dobject+abs(pupil2).*conj(pupil2).*(OP_diff2(:,:,kk));
            dobject(kyl2(kk,:):kyh2(kk,:),kxl2(kk,:):kxh2(kk,:))=dobject(kyl2(kk,:):kyh2(kk,:),kxl2(kk,:):kxh2(kk,:))+abs(pupil).*conj(pupil).*(OP_diff(:,:,kk));
        ...
        err2 = err2+sqrt(sum(sum(((abs(newimSeqLowRes(:,:,i3(ijk)))-abs(im_lowRes_oI)).^2))));
...
%
    ...
    subplot(224);  imagesc((angle(OP_diff(:,:,1))+eps)); axis image; colormap gray; colorbar;
