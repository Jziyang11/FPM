    ...
    %% nine led zheng1
            ...
            for i3=1:9 
                ...
                %[x,y]=find(num_list284==i2);
                    ...
                    im_lowResk = (m/m1)^2*sqrt(abs((newimSeqLowRes2(:,:,i3))))./sqrt(abs(im_lowRes_oI)).*im_lowRes_ori;%�滻���
                    ...
                    err2 = err2+sqrt(sum(sum(((abs(newimSeqLowRes(:,:,i3))-abs(im_lowRes_oI)).^2))));
             end
             ...
             for i3=10:293 
                ...
                %[x,y]=find(num_list284==i2);
                    ...
                    kxc=round((n+1)/2+kx(1,num_list284_3(i3,2))/dkx);%round�Ƕ������ڵ�Ԫ��ȡ��
                    kyc=round((m+1)/2+ky(1,num_list284_3(i3,2))/dky);
                    kyl=round(kyc-(m1-1)/2);kyh=round(kyc+(m1-1)/2);
                    kxl=round(kxc-(n1-1)/2);kxh=round(kxc+(n1-1)/2);
                    lowResFT_ori=(m1/m)^2*objectRecoverFT(kyl:kyh,kxl:kxh).*pupil;
                    im_lowRes_ori = ifft2(ifftshift(lowResFT_ori));
...
am_low = min(min(ampl));
ampl = (ampl-am_low)./(am_high-am_low);
imwrite(ampl,'/home/jnu733/qwm/result/matlab_result/2led/usaf2ampli_284_zheng_fen3.png','png');
...
am_low = min(min(ampl));
ampl = (ampl-am_low)./(am_high-am_low);
imwrite(ampl,'/home/jnu733/qwm/result/matlab_result/2led/usaf2angle_284_zheng_fen3.png','png');
