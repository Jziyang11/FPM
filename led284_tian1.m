...
num_list284 = struct2array(load("num_list2.mat","num_list284"));
newimSeqLowRes2 = zeros(128,128,284);
for k = 1:284
    newimSeqLowRes2(:,:,k) = newimSeqLowRes(:,:,num_list284(k,1))+newimSeqLowRes(:,:,num_list284(k,2));
end 
...
%         end
        ...
        for r=1:2
            kxc=round((n+1)/2+kx(1,num_list284(i3,r))/dkx);%round�Ƕ������ڵ�Ԫ��ȡ��
            kyc=round((m+1)/2+ky(1,num_list284(i3,r))/dky);
            kyl=round(kyc-(m1-1)/2);kyh=round(kyc+(m1-1)/2);
            kxl=round(kxc-(n1-1)/2);kxh=round(kxc+(n1-1)/2);
            lowResFT2(:,:,r)=(m1/m)^2*objectRecoverFT(kyl:kyh,kxl:kxh).*pupil;
            im_lowRes2(:,:,r) = abs(ifft2(ifftshift(lowResFT2(:,:,r))));
        end
...
%         im_lowRes_ori = ifft2(ifftshift(lowResFT_ori));
        ...
        OP_diff = im_lowResk-lowResFT2;
        lowResFT2=fftshift(fft2(im_lowRes2)).*pupil;%ת��Ϊ����Ҷ��
        %OP_diff=(m1/m)^2*lowResFT-lowResFT_ori;
        for kk=1:2
            kxc=round((n+1)/2+kx(1,num_list284(i3,kk))/dkx);%round�Ƕ������ڵ�Ԫ��ȡ��
            kyc=round((m+1)/2+ky(1,num_list284(i3,kk))/dky);
            kyl=round(kyc-(m1-1)/2);kyh=round(kyc+(m1-1)/2);
            kxl=round(kxc-(n1-1)/2);kxh=round(kxc+(n1-1)/2);
...
am_low = min(min(ampl));
ampl = (ampl-am_low)./(am_high-am_low);
imwrite(ampl,'/home/jnu733/qwm/result/matlab_result/2led/usaf2ampli_284_tian1.png','png');
...
am_low = min(min(ampl));
ampl = (ampl-am_low)./(am_high-am_low);
imwrite(ampl,'/home/jnu733/qwm/result/matlab_result/2led/usaf2angle_284_tian1.png','png');
