...
filedir1 = '/home/jnu733/qwm/data/usaf1led/bright/';
%filedir1 = '/home/jnu733/qwm/data/cell8led/';
%filedir2 = '/home/jnu733/qwm/data/usaf1led/dark/';
...
num_list2 = zeros(num_image1+num_image2,2);
look_1_9 = zeros(num_image1+num_image2,2);
bright = [127,128,129,146,147,148,165,166,167];
newimSeqLowRes2 = zeros(128,128,num_image1+num_image2 );
k=1;
while num_list2(num_image1,1)==0
...
Ibk = zeros(Nimg,1);
for mm1 = 1:Nimg
    fn = [filedir2,N{mm1}];  %added for sorting purpose
    disp(fn);
    I = double(imread(fn));
...
dark = [1:293];
bright = [127,128,129,146,147,148,165,166,167];
dark = setdiff(dark,bright);
%newimSeqLowRes2 = zeros(128,128,num_image);
k= num_image1+1;
while num_list2(num_image1+num_image2,1)==0
    i = randi([1,284],1,1,'int16');
    j = randi([1,284],1,1,'int16');

    if i~=j %&& ~ismember(i,bright) && ~ismember(j,bright)
            num_i = dark(i);
            num_j = dark(j);
            num_list2(k,1) = num_i;
            num_list2(k,2) = num_j;
...
num_list = num_list2(num_image1+1:end,:);


na_used=illumination_na(num_list)';
column_sort=sort(na_used);
head_one=column_sort(1,:);
[dis_lit_new,idx_led_new] = sort(head_one);
num_list = num_list(idx_led_new,:);
num_list2(num_image1+1:end,:) = num_list;

image = newimSeqLowRes2(:,:,num_image1+1:end);
image  = image(:,:,idx_led_new);
newimSeqLowRes2(:,:,num_image1+1:end) =image;
...
for tt=1:loop
    ...
    for i3 = 1:num_image1+num_image2
%         i2=seq293(i3);
% 
...
%         end
        ...
        for r=1:2
            ...
            lowResFT2(:,:,r)=(m1/m)^2*objectRecoverFT(kyl:kyh,kxl:kxh).*pupil.*pupil0;
...
%         im_lowRes_ori = ifft2(ifftshift(lowResFT_ori));
        ...
        for kk=1:2
             im_lowResk(:,:,kk) = (m/m1)^2*sqrt(abs((newimSeqLowRes2(:,:,i3))))./sqrt(abs(im_lowRes_oI)).*(im_lowRes2(:,:,kk));%�滻���
        end
        lowResFT=fftshift(fft2(im_lowResk)).*pupil.*pupil0;%ת��Ϊ����Ҷ��
        ...
        for kk=1:2
            ...
            objectRecoverFT(kyl:kyh,kxl:kxh)=objectRecoverFT(kyl:kyh,kxl:kxh)+gamma2*abs(pupil).*conj(pupil).*(OP_diff(:,:,kk))./max(max(abs(pupil)))./(abs(pupil).^2+alpha);
            pupil =  pupil+gamma2/max(max(abs(objectRecoverFT))).*(abs(OFTdown).*conj(OFTdown)).*(OP_diff(:,:,kk))./(abs(OFTdown).^2+beta).*pupil0;
